...
def parse_file(file_object,startNum,endNum,filename,tag):
    ...
    for line in file_object:
        ...
        # (not needed for PFNs or different sqrts calculations)
        ...
        #---------- debug sqrts calcs
        sqrtSDiff.append(this_record['measuredsqrtshat'] - this_record['measuredsqrtshatwphoton'])
        #print('diff: ', sqrtSDiff)
        #print(hadronSqrtSHat.M(),  " ", (hadronSqrtSHat-outgoingPhoton).M(), " ", this_record['measuredsqrtshatwphoton'])

        all_records.append(this_record)

    #make_debug_sqrts_plots(chargedPts,neutronPts,sqrtSDiff,tag,filename)
